...
export const SPELLS = {
  arcane: [
    ...
    /* ── 마법사의 궁극기 ──────────────────────────────────
       여덟째 칸. 다섯 + 공통 둘로 일곱이었고, §4의 여덟 칸 중 마지막
       하나가 비어 있었다 — 마법사는 기예가 없으므로 궁극기도 주문으로
       와야 한다.

       크리틱을 세 번 지났다:
       · **이미 있는 것 다시 팔기.** 서리 폭발이 이미 광역이다. 그래서
         이것은 「더 넓은 서리」가 아니라 **값의 모양이 다른 것**이다:
         피해가 주사위가 아니라 **태운 마나**에서 나온다. 화면의 마나
         숫자가 곧 피해 예고이고, 그러면 「지금 쏠까 더 모을까」가
         주사위가 아니라 계산이 된다. §4의 「신은 수치를 안 속인다」가
         여기서는 규칙 자체가 된다 — 통을 보면 위력을 안다
       · **이 저장소가 한 번 잘라낸 형태.** 잘린 것은 「이중 시전」
         (할인)과 「버티기」(가만히 서 있기)였다. 이것은 할인이 아니고
         (오히려 값이 통 전체다) 서 있는 동사도 아니다
       · **남의 직업 것.** 자원을 전부 태우는 것은 도적의 「급소」와
         닮았지만, 급소는 **하나**를 때리고 이것은 **보이는 전부**를
         때린다. 그리고 급소 뒤에 도적은 다시 모을 수 있고, 폭주 뒤에
         마법사는 열 턴 동안 아무것도 못 모은다

       그 열 턴이 이 주문의 값이다. 「통이 빈다」만으로는 값이 안 된다 —
       계단 앞에서 쏘면 다음 층에서 저절로 찬다. 그래서 회복 자체를
       막는다: 자연 회복도, 물약도, 지팡이도, 모닥불도. 그 열 턴 안에
       싸움이 안 끝나면 마법사는 막대기를 든 사람이다.

       잔향(ECHOES)에 항목이 없는 것은 잊은 것이 아니다 — 마지막
       문장에는 다음 문장이 없다. leaveEcho 가 모르는 id 에서 조용히
       되돌아가므로 규칙도 이미 그렇게 되어 있다. */
