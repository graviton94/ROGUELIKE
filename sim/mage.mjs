...
/* 12레벨 주문이므로 판당 여러 번은 아니다. 다만 **0이면 안 된다** —
   ...
   낮게 둔다.
   ── 문턱을 0.3에서 0으로 내린다 ────────────────────────────
   0.3으로 걸었더니 같은 코드에서 1.4 · 0.4 · 0.3이 나오며 판정이
   뒤집혔다. 이 값은 「12레벨에 닿은 판이 몇이었나」에 통째로 매달려
   있고 그건 배치마다 흔들린다 — 즉 0.3은 게임이 아니라 표본을 재는
   문턱이었다. 물어야 할 것은 처음부터 **0인가 아닌가**였고
   (주석이 그렇게 적혀 있다), 재는 판이 3배치×8판이라 진짜 0이면
   여전히 잡힌다. 간헐적으로 우는 벤치는 안 우는 것만 못하다(§6-6). */
ok(m('surges') > 0, '봇이 폭주를 실제로 쓴다 — 0이면 여덟째 칸은 만들어지지 않은 것이다',
