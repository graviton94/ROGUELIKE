...
/* 문턱 6%. 기준선이 2.5~3.9%로 흔들리고, 함정 한 곳만 되살려도
   ...
   갈리는 자리에. */
ok(ghostPct < 6,
   '깔때기를 안 지나고 사라지는 체력이 6% 미만이다 — 제단의 자발적 지불 말고는 없어야 한다',
   `${ghostPct.toFixed(1)}%`);
/* 이 줄은 지웠다. 우회를 되살린 상태(유령 9.8%)에서도 0.6%로 통과해서,
   벤치 칸을 차지한 채 아무 일도 안 하고 있었다. 통과하는 단언이
   거짓말하는 것이 아예 없는 것보다 나쁘다. 상한은 위의 taken/cap이
   보고, 우회는 유령 비율이 본다 — 이 줄이 볼 것이 남아 있지 않다. */
console.log(`  (참고) 바깥에서 잰 상한 초과 ${over.length}/${hits.length}건 — 판정에는 안 쓴다`);
console.log(bad ? `\n절벽 벤치: ${bad}건 실패\n` : '\n절벽 벤치: 상한이 서 있다\n');
