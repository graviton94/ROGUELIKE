...
{
  /* 「판당 2~3회쯤 쓸 크기인가」(NEXT ①). 쓴 횟수만으로는 못 읽는다 —
     안 쓰는 통과 작은 통이 같은 0으로 찍힌다. 마른 턴이 함께 있어야
     한다: 비시전자는 쓰고 싶은데 못 쓰는 턴이 **있어야** 하고(통이
     작다는 뜻), 시전자보다 그 턴이 많아야 한다. */
  const nDry = NOT.reduce((s, c) => s + meas[c].dry, 0) / NOT.length;
  const cDry = CASTER.reduce((s, c) => s + meas[c].dry, 0) / CASTER.length;
  ok(nDry > cDry, '비시전자가 「쓰고 싶은데 통이 빈」 턴을 시전자보다 많이 만난다 — 작은 통의 정의다',
     `비시전자 ${nDry.toFixed(1)}턴/판 vs 시전자 ${cDry.toFixed(1)}턴/판`);
}
{
