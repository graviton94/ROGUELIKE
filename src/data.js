...
/* ── 다섯 신 ──────────────────────────────────────────────
   ...
   그래서 칸이 둘이다:
     ...
     say   신이 **말하는** 것. 서약 화면에 뜨는 문장
     real  실제로 일어나는 것. 겪고 나서 기록에 남는다

   둘이 어긋나는 정도가 신마다 다르다. 어떤 신은 조금 부풀리고, 어떤
   신은 정반대를 말한다. 첫 판에는 어느 쪽인지 알 길이 없다 — 그게
   속는다는 것의 뜻이다.

   말투는 **명령형**이다(§2). 짧고, 이유를 안 댄다.

   ── 다섯이 각자 이미 재고 있는 깔때기 하나씩을 먹는다 ────
   새 배관을 거의 안 만든다. lightTurns · 처치 · sneaked · 금화 ·
   hurtPlayer 가 그대로 신앙심의 입구가 된다.

   숫자는 아직 안 박았다 — 순서 2-2(신앙심과 계율)에서 잰다. */
export const GODS = [
  { id:'ember', n:'꺼지지 않는 것', face:'불',
    call:'불을 지켜라. 어둠은 내 것이 아니다.',
    say:'등불이 밝은 동안 주는 피해가 오른다.',
    real:'등불이 밝은 동안 피해가 오른다 — 그리고 **기름이 두 배로 탄다.**',
    vow:'어둠 속에서 싸우지 마라',
    lore:'그는 빛을 준 적이 없다. 태울 것을 줬을 뿐이다.' },

  { id:'blood', n:'피를 세는 자', face:'피',
    call:'세어라. 나는 숫자만 안다.',
    say:'재운 것마다 몸이 조금씩 돌아온다.',
    real:'재운 것마다 몸이 돌아온다 — **물약은 이제 아무 일도 안 한다.**',
    vow:'병에 든 것을 마시지 마라',
    lore:'세는 쪽은 그이지 당신이 아니다.' },

  { id:'hush', n:'침묵의 어머니', face:'침묵',
    call:'조용히 하여라.',
    say:'들키지 않은 채 걸을수록 손이 빨라진다.',
    real:'들키지 않은 채 걸을수록 손이 빨라진다 — **소리치면 그만큼 잃는다.**',
    vow:'소리치지 마라',
  ...
  { id:'scale', n:'저울을 든 자', face:'저울',
    call:'쌓지 마라. 흐르게 하여라.',
    say:'쓴 금화만큼 값이 내려간다.',
    real:'쓴 금화만큼 값이 내려간다 — **쌓아 두면 그만큼 삭는다.**',
    vow:'금화를 쌓아 두지 마라',
  ...
  { id:'scar', n:'상처를 세는 자', face:'상처',
    call:'견뎌라. 아물지 마라.',
    say:'흉터가 깊을수록 무겁게 친다.',
    real:'흉터가 깊을수록 무겁게 친다 — **그리고 흉터를 지울 수 없다.**',
    vow:'흉터를 지우지 마라',
