...
/* ── 기준선 ────────────────────────────────────────────────
   ...
   ── 앞의 둘은 **흘러간 기준선**이었다 ──────────────────────
   순서 3 작업에 들어와서 이 린트가 네 줄 다 울었다. 무엇을 내가
   나쁘게 만들었는지 가리려고 세션 시작 커밋(94afede)에서 같은 자로
   다시 재 봤더니:

       94afede   30초과 24 · 15초과 67 · 최악 167 · 최장 847
       BASE      30초과 23 · 15초과 66 · 최악 167 · 최장 840

   즉 앞의 둘은 **내가 오기 전에 이미 넘어가 있었다.** 어느 커밋이
   함수 하나를 15 위로, 하나를 30 위로 밀면서 이 표를 안 고쳤다.
   그 뒤로 이 린트는 「아무도 안 만든 실패」를 계속 인쇄했고, 그러면
   다음 사람은 네 줄 전부를 배경 소음으로 읽는다 — 우는 벤치가 안 우는
   것만 못해지는 자리가 정확히 여기다.

   그래서 앞의 둘은 94afede 실측값으로 바로잡는다(올리는 것이지만
   내가 올린 것이 아니고, 이 주석이 그 증거다). 뒤의 둘은 순서 3에서
   실제로 내가 172·849까지 밀었다가 pump 에서 judgestFx 를 빼내
   167·837로 되돌렸다 — 최장은 원래 기준선보다 낮아졌으므로 내려
   적는다.

   여기서 **BASE 를 올려 적으려면 반드시 이런 문단이 있어야 한다.**
   숫자만 고치는 커밋은 회귀를 지우는 커밋이다. */
const BASE = {
  ...
  longest: 837,    // 가장 긴 함수의 줄 수 (같은 pump — judgestFx 를 빼내 849에서)
};
