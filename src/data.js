...
export const RELICS = [
  ...
  { id:'famine',  n:'끝없는 허기',   spr:'sword',  k:'famine',  v:20,   fused:true, crack:'③',
    lore:'허기는 옮는다. 이것을 든 사람의 배낭은 늘 비어 있었다.',
    t:'처치할 때마다 체력 +20 — **천장을 넘겨서**. 물약 효과 두 배. 기름을 세 배로 태운다.' },
  { id:'paradox', n:'모순의 룬',     spr:'wand',   k:'paradox', v:0.22, fused:true, crack:'②',
    lore:'읽으면 뜻이 맞고, 다시 읽으면 안 맞는다. 세 번째부터는 안 읽게 된다.',
    t:'주문이 **공짜가 된다** — 마나가 자원이 아니게 된다. 주문 피해 −22%. 근접 피해 +20%.' },
  { id:'oracle',  n:'눈먼 예언자',   spr:'scroll', k:'oracle',  v:3,    fused:true, crack:'③',
  ...
  { id:'march',   n:'울리는 진군',   spr:'amulet', k:'march',   v:3,    fused:true, crack:'①',
...
export const RELIC_CRACKS = {
  pact:    { c:'②', at:['crit', 55],   n:'계약이 파기된다',
             t:'치명타는 남고 **최대 체력이 돌아온다.** 서명한 쪽이 먼저 지운 셈이다.' },
  echo:    { c:'①', at:['combo', 11],  n:'종이 멎지 않는다',
             t:'연격 **3**이면 울리고, 울린 것이 **또 한 번 울릴 수 있다.**' },
  hunger:  { c:'②', at:['kill', 130],   n:'자루가 배부르다',
             t:'회복은 남고 **기름을 두 배로 태우지 않는다.**' },
  mirror:  { c:'①', at:['hit', 130],    n:'거울이 깊어진다',
             t:'되돌리는 몫이 35%가 아니라 **전부다.**' },
  eye:     { c:'③', at:['floor', 8],   n:'눈이 감기지 않는다',
             t:'지도만이 아니다 — **몬스터가 선 자리가 층 내내 보인다.** 어둠이 정보가 아니게 된다.' },
  glove:   { c:'③', at:['trap', 16],    n:'함정이 전리품이 된다',
             t:'밟은 함정은 **피해 대신 물건을 뱉는다.** 함정이 상자가 된다.' },
  ember:   { c:'①', at:['floor', 9],   n:'항아리가 비지 않는다',
             t:'모닥불을 **세 번** 쓰고, **모닥불이 없는 층이 없어진다.**' },
  scale:   { c:'①', at:['kill', 100],   n:'접시가 기운다',
             t:'문턱이 **절반**으로 올라가고 피해는 **+120%**가 된다.' },
  twin:    { c:'②', at:['spell', 150],  n:'룬이 어긋나지 않는다',
             t:'비용은 그대로 −2, **주문 피해가 깎이지 않는다.**' },
  thief:   { c:'③', at:['floor', 11],   n:'모래가 위로 흐른다',
             t:'층의 여유가 줄지 않는다 — **35% 늘어난다.** 부호가 뒤집힌다.' },
  bone:    { c:'①', at:['kill', 170],   n:'실이 끝나지 않는다',
             t:'구슬을 서른까지 세지 않는다. **끝까지 센다.**' },
  chain:   { c:'③', at:['hit', 150],    n:'사슬이 대신 맞는다',
             t:'층마다 **첫 피격이 언제나 0이다.** 처음 한 대는 사슬이 받는다.' },
  compass: { c:'③', at:['trap', 20],   n:'아래를 가리킨다',
             t:'함정은 여전히 안 보인다. 대신 **계단이 층 내내 보인다.**' },
  gut:     { c:'①', at:['gulp', 20],   n:'위장이 끝이 없다',
             t:'물약 효과가 두 배가 아니라 **세 배**가 된다.' },
  reckless:{ c:'①', at:['crit', 45],   n:'더 멀리 간다',
             t:'명중은 여전히 −15%. 치명 배율이 **×2.6**이 된다.' },
  vow:     { c:'③', at:['floor', 9],   n:'아무도 당신을 부르지 않는다',
             t:'몬스터가 **소리로는 깨어나지 않는다** — 보아야 깨어난다.' },
  lamp:    { c:'①', at:['floor', 8],   n:'등이 안 꺼진다',
             t:'기름이 줄지 않고, 좁아지던 2칸이 **넓어지는 2칸이 된다.**' },
  nighteye:{ c:'③', at:['floor', 9],   n:'어둠이 더 잘 보인다',
             t:'어둠에서 무뎌지지 않는 정도가 아니다 — **불이 꺼진 동안 피해 +25%.**' },
  everflame:{ c:'②', at:['floor', 7],  n:'불이 살을 태우지 않는다',
             t:'불빛은 그대로 5칸. **최대 체력이 돌아온다.**' },
  moth:    { c:'③', at:['floor', 7],   n:'나방이 먼저 난다',
             t:'보이는 것으로 끝나지 않는다 — **층마다 시설이 하나 더 선다.** 나방이 앉을 자리를 만든다.' },
  knot:    { c:'①', at:['trap', 12],    n:'매듭이 전부 통한다',
             t:'거미줄과 구덩이만이 아니라 **모든 함정이 통하지 않는다.**' },
  toll:    { c:'②', at:['gold', 14000],  n:'뱃사공이 잊는다',
             t:'금화는 두 배 그대로, **층을 내려가도 뺏기지 않는다.**' },
  brand:   { c:'①', at:['elite', 26],  n:'전부 낙인이 된다',
             t:'정예에게 주는 피해가 **+100%**가 된다.' },
  quill:   { c:'①', at:['gold', 8000],  n:'적힌 대로 된다',
             t:'즉시 판별에 더해, **적힌 것이 나쁘게 나오지 않는다** — 주운 것에 저주가 붙지 않는다.' },
  grudge:  { c:'①', at:['hit', 190],    n:'앙심은 잊지 않는다',
             t:'층이 바뀌어도 **지워지지 않고**, 최대 **+120%**까지 쌓인다.' },
  seed:    { c:'①', at:['floor', 9],   n:'돌이 더 자란다',
             t:'층을 내려갈 때 방어 **+2**.' },
  wick:    { c:'①', at:['gulp', 14],   n:'심지가 길어진다',
             t:'타는 범위가 **두 칸**, 화상 피해가 **두 배**.' },
  drum:    { c:'①', at:['combo', 14],  n:'북이 멎지 않는다',
             t:'맞아도 **연격을 하나도 잃지 않는다.**' },
  grip:    { c:'③', at:['kill', 120],   n:'힘이 민첩을 대신한다',
             t:'민첩은 6인 채로, **민첩이 하던 일을 힘이 한다.**' },
  specs:   { c:'①', at:['spell', 130],  n:'더 크게 보인다',
             t:'높은 쪽이 둘 다가 되는 데서 그치지 않고 **거기에 +4.**' },
  acro:    { c:'②', at:['floor', 9],   n:'발이 힘을 안 뺏는다',
             t:'민첩 +6은 그대로, **힘이 깎이지 않는다.**' },
  bull:    { c:'①', at:['kill', 110],   n:'심장이 커진다',
             t:'체질이 **+10**이 된다.' },
  mask:    { c:'③', at:['gold', 11000],  n:'가면이 웃어 준다',
             t:'매력 18에 더해 — **상인이 부르는 값이 절반이 된다.**' },
  ballast: { c:'②', at:['floor', 11],   n:'배가 뜬다',
             t:'가장 낮은 것이 가장 높은 것과 같아지고, **최대 체력이 돌아온다.**' },
  ...
  paradox: { c:'②', at:['fused', 0],   n:'마나가 자원이 아니다',
             t:'주문이 **공짜**다.' },
  oracle:  { c:'③', at:['fused', 0],   n:'눈이 없어야 보인다',
             t:'지도와 **몬스터가 선 자리**가 층에 들어서는 순간 전부 보인다.' },
  ledger:  { c:'②', at:['fused', 0],   n:'장부가 빚을 안 진다',
             t:'금화 두 배와 즉시 판별은 그대로, **층을 내려가도 뺏기지 않는다.**' },
  march:   { c:'①', at:['fused', 0],   n:'행군이 멎지 않는다',
             t:'연격 3이면 한 번 더, 맞아도 깎이지 않는다.' },
};
...
export const CRACK_WORDS = {
  ...
  gold: n => `금화 ${n}을 벌면`,  floor: n => `낀 채 ${n}층을 내려가면`,
  fused:() => '융합된 순간부터',
};
