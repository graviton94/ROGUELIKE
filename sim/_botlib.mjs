...
function runBot(race, cls, clear, opt = {}) {
  ...
  while (G.running && guard++ < 60000) {
    ...
    /* ── 서약은 「닫아 버리는 화면」이 아니다 ────────────────
       위의 화면 탈출기가 선택 화면을 스무 번 만에 그냥 닫아 버리면,
       봇은 아무것도 안 든 채로 판을 마친다. 실제로 그렇게 60판을
       굴린 적이 있다.

       무엇을 고르는지가 아니라 「고른다」가 중요하므로 셋 중 하나를
       무작위로 집는다. 편향된 정책은 나중에 A/B 로 따로 잰다. */
    if (G.screen === 'pledge' && Game.pledgeDue(G.depth)) {
      /* 봇이 서약을 모르는 채로 열 판을 굴렸더니 평균 5.7층이 나왔다 — 사실상
