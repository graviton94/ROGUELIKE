...
export const HAND = {
  // 아이템은 왼쪽 아래(자루)에서 오른쪽 위(끝)로 눕혀 그려져 있습니다.
  // 그 방향이 몸 바깥을 향하도록 방향마다 돌리고 뒤집습니다.
  down:  { x: 4,  y: 13, rot: -Math.PI / 2, flip: false, under: false },
  right: { x: 10, y: 12, rot: 0,            flip: false, under: false },
  left:  { x: 6,  y: 12, rot: 0,            flip: true,  under: true  },
  up:    { x: 11, y: 13, rot: 0,            flip: false, under: true  },
};

/* 아이템 스프라이트 안에서 자루가 있는 자리, 역시 픽셀 좌표. 무기는
   전부 왼쪽 아래에서 오른쪽 위로 눕혀 그렸으므로 자루는 늘 왼쪽 아래
   모서리 근처입니다. */
export const GRIP = { x: 2, y: 14 };
