...
/* 셈이 몸마다 따로인가. 하나뿐이면 도끼 연격이 특성을 **깎는다**. */
{
  ...
  const ms = stand(p, [[1, 0], [0, 1], [1, 1]]);
  Game.cast('combo');
  /* 「셋 다 정확히 1」로 잡았더니 세 번에 한 번씩 울었다 — 도끼는
     양옆까지 한 번 더 치므로 한 몸이 두 번 맞고, 그 둘째가 빗맞으면
     그 몸의 셈이 0 으로 돌아간다. 간헐적으로 우는 벤치는 안 우는 것만
     못하다(§6).

     물어야 할 것은 「셈이 **영웅에게 하나**인가 몸마다인가」다. 하나면
     부채꼴 뒤에 셈을 가진 몸이 최대 하나뿐이다 — 둘 이상이면 몸마다인
     것이 증명된다. */
  const held = ms.filter(m => (m.chain || 0) > 0).length;
  ok(held >= 2,
     '   셈은 몸마다 따로다 — 하나뿐이면 부채꼴 뒤에 한 몸만 남는다',
     `셈을 가진 몸 ${held}/3 (${ms.map(m => m.chain || 0).join(' ')})`);
}
