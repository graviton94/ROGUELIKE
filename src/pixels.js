...
export const SPRITES = {
  /* player — recolored per class at bake time via tint keys (16x16 Pokemon GBC style) */
  hero: [
    '....nnnnnnnn....',
    '...nnWWWWWWnn...',
    '..nnWwwwwwwWnn..',
  ...
  /* undead & horrors (16x16 GBC style) */
  wraith: [
    'k.....kcceek....',
    '.....kcceekbbk..',
    '.....ekkcbbk....',
  ...
  /* dragons & the deep (16x16 GBC style) */
  dragon: [
    '................',
    '....kNk..dok....',
    '...rNdnk..kkrk..',
  ...
  /* items & equipment: Minecraft & GBC 16x16 style */
  potion: [
    '................',
    '......kNNNk.....',
    '......knnnk.....',
  ...
  /* ── 마흔 유물과 일곱 이름 ────────────────────────────────
     ...
     읽히게 했다.                                                */
  /* ── 유물 여섯을 더 그렸다 ─────────────────────────────────
     세 번 그려서 세 번 실루엣 린트에 걸렸다. 첫 판은 「둥근 가족이
     붐비니 안 둥글게 그린다」였고 46쌍 → 50쌍, 두 번째는 걸린 셋을
     다시 그려서 **더 나빠졌다**(50 → 53). 두 번 다 짐작으로 자리를
     골랐다.

     그래서 자에게 물었다. 마흔일곱이 8×8 안에서 **어느 칸을 쓰는지**
     세어 보니(sim/iconfit.mjs --heat) 답이 한눈에 나왔다:
     ...
     **아래 두 줄과 오른쪽 끝이 비어 있다.** 마흔일곱이 전부 왼쪽 위에
     모여 있고, 아무도 바닥에 앉지 않는다. 그리고 붐비는 것을 정하는
     것은 「가족」이 아니라 **칸 수**였다 — 8×8을 구우면 채운 칸마다
     사방 한 줄이 외곽선으로 붙으므로, 열세 칸을 넘으면 무엇을 그려도
     140/256을 넘고 서로 닮는다. 실패한 넷은 15~19칸이었고, 통과한
     둘은 13칸과 14칸이었다.

     그래서 규칙 둘로 다시 그렸다: **열세 칸 안에서, 아래나 오른쪽에
     앉힌다.** 여섯이 87~136으로 들어왔고 실패 쌍은 0이다.
     (거머리는 사선, 나팔은 아래로 벌어지는 곡선, 역병은 왼쪽 아래
     얼룩과 떠오르는 점, 관은 오른쪽 기둥에서 꺾이는 엘보, 재는 바닥
     무더기, 걸음은 바닥의 세 줄기다.)                              */
  r_leech: [
    '.gg.....',
    'g..g....',
  ...
  /* ── 네 직업의 넷 ─────────────────────────────────────────
     같은 두 규칙으로 그렸다(열세 칸 안에서, 아래나 오른쪽에).
     sim/iconfit.mjs 로 후보 열둘을 한 번에 재서 고른 것이고, 넷 다
     가장 닮은 이웃이 0.60 아래다.                                */
  r_third: [
    '........',
    '........',
  ...
  /* 가지 치는 것 — 날이 아래로 걸린 도끼. 여덟 계열 중 도끼에만
     이름 붙은 것이 없었다. 실루엣 지도(iconfit --heat)에서 제일 찬
     칸이 아래 줄과 오른쪽 아래 모서리라, 자루를 왼쪽 위에서 내리고
     날을 오른쪽 **아래**에 건다 — 마흔일곱이 쓰는 왼쪽아래→오른쪽위
     대각선을 뒤집는 것이 이 자에서는 가장 싸게 벌어지는 방향이다. */
  u_pruner: [
    'N.......',
    'N.......',
    '.N......',
  ...
  /* 강을 재던 것 — 눈금이 새겨진 장대. 곧은 세로 하나에 왼쪽으로만
     눈금이 삐져나온다. 세로로 곧은 것은 못 박는 자 하나뿐이었고
     그쪽은 위가 넓은 T자다. */
  u_sounding: [
    '......s.',
    '.....ws.',
