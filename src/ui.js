...
function renderSpellRow() {
  ...
  const arts = all.filter(s => s.art), spells = all.filter(s => !s.art);
  /* ── 여섯 칸까지는 한 줄에 같이 놓는다 ────────────────────
     기예와 주문을 갈라 놓은 이유는 「자원이 다르면 줄도 다른 편이
     낫다」였고, 그 판단은 팔라딘의 아홉 칸이 한 줄에서 「치유」를
     「치듀」로 자르던 시절에 내려졌다. 지금은 slotCols() 가 접으므로
     자르는 문제는 없고, 남은 것은 **높이**다: 320px에서 줄 하나가
     38px이고 그 38px이 곧 지도 38px이다(화면의 7%).

     공통 치유 둘이 열리면서 주문 줄이 없던 세 직업(전사·도적·레인저)에
     줄이 하나 생겨 지도 몫이 32% → 20%로 떨어졌다. 여섯 칸이면 320px
     에서 칸이 46px — 손가락(44px)보다 넓다. 그러니 갈라 둘 이유가
     없어진 구간에서는 합친다.

     자원이 다르다는 신호는 줄이 아니라 **칸**이 이미 갖고 있다:
     기예는 테두리가 주황이고 값 점이 초록(기력), 주문은 파랑(마나)이다.

     줄 수는 직업이 정하고 판 중에 안 바뀐다 — 잠긴 칸도 자리를
     차지하므로 칸 수가 레벨로 변하지 않는다. 그래서 나타났다 사라지며
     지도를 미는 문제는 여기서도 안 생긴다. */
  /* ── 그리고 합쳐 봤더니 글자가 잘렸다 ──────────────────────
     실제로 해 보고 되돌린 자리다. 여섯 칸을 한 줄에 놓으면 320px에서
     칸이 46px이고 지도 몫이 20% → **29%**로 돌아온다. 그런데 그 46px
     안에 아이콘(13) + 이름 + 값 점이 안 들어간다 — 여섯 칸 **전부**가
     잘렸다(「연격2」 「소용돌이5」 「되감기3」…).

     둘 다 벤치 실패지만 같은 무게가 아니다. 잘린 글자는 §0이 이름을
     붙여 가장 자주 고쳐 온 결함이고(「치유」가 「치듀」가 된 그 일이
     이 벤치를 만들게 했다), 지도 몫은 **읽을 수는 있는데 좁은** 것이다.
     그래서 두 줄로 되돌린다.

     남은 38px을 찾을 자리는 줄이 아니라 HUD(103px) · 로그(40px) ·
     방향판(128px) 쪽이고, 그건 배치를 다시 짜는 일이다 — NEXT.md 에
     측정값과 함께 남겼다. */
  paintSlotRow($('art-row'), arts);
  paintSlotRow($('spell-row'), spells);
}
