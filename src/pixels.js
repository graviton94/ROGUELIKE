...
/* 자루는 언제나 무기 그림의 맨 아래 가운데입니다. 길이가 무기마다
   달라도 이 한 점만 손에 맞추면 되므로 상수로 둡니다. */
export const GRIP = { x: WEAPON_W / 2, y: WEAPON_H - 1 };
...
export function bakeAll() {
  for (const [name, art] of Object.entries(WEAPON))
    baked.set(`weapon:${name}`, bakeWeapon(art));
