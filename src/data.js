...
export const TASKS = [
  { id:'key',  n:'열쇠를 문 것', from:4, w:3,
    intro:'계단이 잠겨 있다. 이 층의 무언가가 열쇠를 삼켰다.',
    done:'열쇠가 나왔다. 계단이 열린다.',
    hint:'열쇠를 문 것을 찾아라 — 이 층 어딘가에 있다.' },
  /* 열쇠 쪽은 재 보니 **걷기를 더 시켰다**(판당 1191턴 → 1529턴).
     찾는 일이니 당연하다. 걷기 자체를 줄이는 과업이 따로 있어야
     한다 — 이쪽은 계단을 처음부터 보여 주고 시계를 반으로 줄인다.
     잠그는 것이 아니라 **재촉하는** 과업이라 `open`이 필요 없다. */
  { id:'fall', n:'무너지는 층', from:3, w:3, rush:true,
    intro:'천장에서 흙이 떨어진다. 이 층은 오래 못 간다 — 계단은 저기다.',
    done:'', hint:'' },
];
