/* _gearbase.mjs — sim/tags.mjs 의 기준선을 다시 뜬다.
   ...
   ── 다시 뜬 기록 ──
   2026-08-18, ⑤-유물 (4). 대가 없는 여섯에 대가를 붙였다. 다시 뜨기
   **전에** tags.mjs 로 달라진 칸을 전부 인쇄해서 **열 칸이 의도한 열
   칸인지** 확인했다(메아리의 종 maxhpPct · 거울 방패 stealth · 불씨
   항아리 lightR · 뼈 목걸이 hit · 앙심 spellPow, 각각 평상/크랙 둘).
   그 절차를 안 밟고 다시 뜨면 「의도한 변경」과 「조용한 변경」이 같은
   커밋에 섞이고, 그러면 이 파일이 있는 이유가 없어진다.
